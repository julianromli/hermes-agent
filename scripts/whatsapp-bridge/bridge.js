...
// Cache directories: the Python gateway passes profile-aware paths via env.
...
const MESSAGE_LOG_FILE = path.join(process.env.HOME || '~', '.hermes', 'whatsapp', 'messages.jsonl');

// Self-hash reported in /health so the gateway can detect stale bridge processes.
let SCRIPT_HASH = '';
try {
...
function appendMessageLog(event) {
  try {
    const line = JSON.stringify(event) + '\n';
    appendFileSync(MESSAGE_LOG_FILE, line, { encoding: 'utf8' });
  } catch (err) {
    // Silently ignore logging errors to avoid disrupting message flow
  }
}
...
async function startSocket() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (qr) {
      writeFileSync('/tmp/wa-qr-string.txt', qr);
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // skipQueue gates messageQueue.push (bot command processing) but NOT
      // appendMessageLog (JSONL logging). All group messages must be logged
      // for cron summaries, even from non-allowlisted senders. The allowlist
      // is a security gate for bot commands, not for message observation.
      let skipQueue = false;
      ...
      if (!msg.key.fromMe) {
        ...
        if (!matchesAllowedUser(senderId, ALLOWED_USERS, SESSION_DIR)) {
          // allowlist_mismatch logging disabled to reduce log noise (audit 2026-07-25).
          // Do NOT continue — still log to JSONL for group summaries.
          // Only skip the in-memory queue (bot command processing).
          skipQueue = true;
        }
      }
...
// Group participant management (kick/add/promote/demote)
app.post('/group/:id/participants', async (req, res) => {
  if (!sock || connectionState !== 'connected') {
    return res.status(503).json({ error: 'Not connected to WhatsApp' });
  }
  ...
  try {
    const result = await sock.groupParticipantsUpdate(chatId, participants, action);
    res.json({ success: true, action, count: participants.length, result });
  } catch (err) {
